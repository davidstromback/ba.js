import type { ElementAttributes, VDOMChild, VDOMElement } from "../vdom";
...
export function isElementVDom(vdom: VDOMElement): vdom is VDOMElement<string> {
  return typeof vdom.type === "string";
}
...
export function mountElement(
  ...
  if (typeof vdom.props.ref !== 'undefined'){
    vdom.props.ref.current = domNode
  }

  mountChildren(children, sanitizeVDOMChildren(props.children));
}

export function updateElement(
  element: ElementInstance,
  nextVDom: VDOMElement<string, ElementAttributes>
): void {
  const { vdom: prevVDom, parent, domNode, children } = element;
  const { type: prevType, props: prevProps } = prevVDom;
  const { type: nextType, props: nextProps } = nextVDom;
  ...
  if (typeof nextVDom.props.ref !== 'undefined'){
    nextVDom.props.ref.current = domNode
  }
  ...
  updateChildren(children, sanitizeVDOMChildren(nextProps.children));
}
...
export function unmountElement(element: ElementInstance): void {
  const { domNode, vdom } = element;

  if (typeof vdom.props.ref !== 'undefined'){
    vdom.props.ref.current = undefined
  }
